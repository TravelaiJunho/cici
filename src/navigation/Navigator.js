...
const createScreen = (navigator, component, name, title = null, gestureEnabled = true, headerShown = false) =>
    <navigator.Screen name={name}
                      component={component}
                      options={{
                          tabBarLabel: title,
...
const createTabScreen = (navigator, component, name, title = null, onScroll = null, onTabFocus = null) =>
    <navigator.Screen name={name}
                      component={component}
...
const AuthStack = _ => {
    ...
    return (
        <Navigator.Navigator initialRouteName={Screen.STACK_LOGIN.LOGIN}>
            ...
            {createScreen(Navigator, Authentication, Screen.STACK_LOGIN.AUTHENTICATION)}
            {createScreen(Navigator, ResetPassword, Screen.STACK_LOGIN.RESET_PASSWORD)}
            {createScreen(Navigator, ChangePassword, Screen.STACK_LOGIN.CHANGE_PASSWORD)}
        </Navigator.Navigator>);
}
...
const HomeStack = _ => {
    ...
    return (
        <Navigator.Navigator initialRouteName={Screen.STACK_HOME.HOME}>
            ...
            {createScreen(Navigator, KangDanielMedia, Screen.STACK_HOME.KANGDANIEL_MEDIA)}
        </Navigator.Navigator>);
}
...
const MoreStack = _ => {
    ...
    return (
        <Navigator.Navigator initialRouteName={Screen.STACK_MORE.MORE}>
            ...
            {createScreen(Navigator, ChangePassword, Screen.STACK_MORE.CHANGE_PASSWORD)}
            {/* Member */}
            {createScreen(Navigator, MembershipGuide, Screen.STACK_MORE.MEMBERSHIP_GUID)}
            {createScreen(Navigator, MembershipInfo, Screen.STACK_MORE.MEMBERSHIP_INFO)}
            {createScreen(Navigator, RegularMemberApply, Screen.STACK_MORE.REGULAR_MEMBER_APPLY)}
            {/* Service */}
...
export const CommunityTabNavigator = ({onScroll = null, onTabFocus = null}) => {
    const Navigator = createMaterialTopTabNavigator();
    return (
        <Navigator.Navigator backBehavior={"none"}
                             swipeEnabled={Common.checkIOS()}
                             tabBar={props => <TabCommunityBar {...props}
                                                               isFix={true}/>}>
            {createTabScreen(Navigator, TabTalkTalk, Screen.STACK_COMMUNITY.TAB_TALK_TALK, 'Talk Talk', onScroll, onTabFocus)}
            {createTabScreen(Navigator, TabFromDaniel, Screen.STACK_COMMUNITY.TAB_FROM_DANIEL, 'From Daniel', onScroll, onTabFocus)}
            {createTabScreen(Navigator, TabToDaniel, Screen.STACK_COMMUNITY.TAB_TO_DANIEL, 'To Daniel', onScroll, onTabFocus)}
        </Navigator.Navigator>);
}
...
export const NoticeTabNavigator = ({onTabFocus = null, useHeader=true}) => {
    const Navigator = createMaterialTopTabNavigator();
    return (
        <Navigator.Navigator backBehavior={"history"}
                             pressOpacity={1}
                             swipeEnabled={Common.checkIOS()}
                             tabBar={props => <TabNoticeBar {...props} isFix={false} useHeader={useHeader}/>}
        >
            {createTabScreen(Navigator, TabNoticeForNotice, Screen.STACK_NOTICE.TAB_NOTICE, 'Notice', null, onTabFocus)}
            {createTabScreen(Navigator, TabSchedule, Screen.STACK_NOTICE.TAB_SCHEDULE, 'Schedule', null, onTabFocus)}
            {createTabScreen(Navigator, TabEventForNotice, Screen.STACK_NOTICE.TAB_EVENT, 'Event', null, onTabFocus)}
            {createTabScreen(Navigator, TabMediaForNotice, Screen.STACK_NOTICE.TAB_MEDIA, 'Media', null, onTabFocus)}
            {createTabScreen(Navigator, TabGoodsForNotice, Screen.STACK_NOTICE.TAB_GOODS, 'Goods', null, onTabFocus)}

        </Navigator.Navigator>);
}

export const MemberShipTabNavigator = ({onScroll = null, onTabFocus = null, useHeader=true}) => {
    const Navigator = createMaterialTopTabNavigator();
    return (
        <Navigator.Navigator backBehavior={"none"}
                             swipeEnabled={Common.checkIOS()}
                             tabBar={props => <TabMemberShipBar {...props}
                                                                isFix={true}
                                                                useHeader={useHeader}/>}>
            {createTabScreen(Navigator, TabNoticeForMemberShip, Screen.STACK_MEMBERSHIP.TAB_NOTICE, 'Notice', null, onTabFocus)}
            {createTabScreen(Navigator, TabMediaForMemberShip, Screen.STACK_MEMBERSHIP.TAB_MEDIA, 'Media', null, onTabFocus)}
            {createTabScreen(Navigator, TabTalk, Screen.STACK_MEMBERSHIP.TAB_TALK, 'Talk', null, onTabFocus)}
            {createTabScreen(Navigator, TabEventForMemberShip, Screen.STACK_MEMBERSHIP.TAB_EVENT, 'Event', null, onTabFocus)}
        </Navigator.Navigator>);
}
...
const BottomTabNavigator = _ => {
    ...
    return (
        <Navigator.Navigator backBehavior={"none"}
            ...
            {createScreen(Navigator, Community, Screen.TAB_BOTTOM.COMMUNITY, "Community")}
            {createScreen(Navigator, Notice, Screen.TAB_BOTTOM.NOTICE, "Notice")}
            {createScreen(Navigator, MemberShip, Screen.TAB_BOTTOM.MEMBERSHIP, "Membership")}
    ...
    return (
        <NavigationContainer ref={navRef}
            ...
            <Navigator.Navigator>
                ...
                {createScreen(Navigator, PostDetail2, Screen.SCREEN_ACTIVITY.POST_DETAIL2)}
                {/* Comment */}
                {createScreen(Navigator, Comment, Screen.SCREEN_ACTIVITY.COMMENT)}
                {/* Notice */}
                {createScreen(Navigator, NoticeDetail, Screen.SCREEN_ACTIVITY.NOTICE_DETAIL)}
                {/* Media  */}
                {createScreen(Navigator, MediaImage, Screen.SCREEN_ACTIVITY.MEDIA_IMAGE, null, false)}
                {createScreen(Navigator, MediaWallpaper, Screen.SCREEN_ACTIVITY.MEDIA_WALLPAPER, null, false)}
                {createScreen(Navigator, MediaVideo, Screen.SCREEN_ACTIVITY.MEDIA_VIDEO, null, false)}
                {createScreen(Navigator, MediaYoutube, Screen.SCREEN_ACTIVITY.MEDIA_YOUTUBE, null, false)}
                {createScreen(Navigator, MediaAudio, Screen.SCREEN_ACTIVITY.MEDIA_AUDIO, null, false)}
                {/* Alarm List */}
                {createScreen(Navigator, AlarmList, Screen.SCREEN_ACTIVITY.ALARM_LIST)}
                {/* Member */}
                {createScreen(Navigator, RegularMemberApply, Screen.STACK_MORE.REQUEST_MEMBER)}
                {/* Terms */}
                {createScreen(Navigator, TermsPolicies, Screen.STACK_MORE.TERMS_POLICIES)}
                {/* Service */}
                {createScreen(Navigator, PostInquire, Screen.SCREEN_ACTIVITY.POST_INQUIRE)}
                {createScreen(Navigator, PostSuggest, Screen.SCREEN_ACTIVITY.POST_SUGGEST)}
                {createScreen(Navigator, InquireDetail, Screen.SCREEN_ACTIVITY.INQUIRE_DETAIL)}
                {createScreen(Navigator, SuggestDetail, Screen.SCREEN_ACTIVITY.SUGGEST_DETAIL)}
                {/* Banner */}
                {createScreen(Navigator, BannerDetail, Screen.SCREEN_ACTIVITY.BANNER_DETAIL)}
                {/* Event */}
                {createScreen(Navigator, EventDetail, Screen.SCREEN_ACTIVITY.EVENT_DETAIL)}
                ...
                {/* Viewer*/}
                {createScreen(Navigator, MediaViewer, Screen.SCREEN_ACTIVITY.MEDIA_VIEWER)}
                {createScreen(Navigator, ImageViewer, Screen.SCREEN_ACTIVITY.IMAGE_VIEWER)}
                {/* Goods */}
                {createScreen(Navigator, GoodsDetail, Screen.SCREEN_ACTIVITY.GOODS_DETAIL)}
                {/* Vote */}
                {createScreen(Navigator, PostVote, Screen.SCREEN_ACTIVITY.POST_VOTE)}
                {/* Public BroadCasting */}
                {createScreen(Navigator, PostPublicBroadCasting, Screen.SCREEN_ACTIVITY.POST_PUBLIC_BROADCAST)}
                {/* Dev */}
                {createScreen(Navigator, TestPageScreen, Screen.SCREEN_ACTIVITY.TEST_PAGE)}
                {createScreen(Navigator, AuthStack, Screen.SCREEN.LOGIN_RE, null, false)}
            </Navigator.Navigator>
        </NavigationContainer>);
}
