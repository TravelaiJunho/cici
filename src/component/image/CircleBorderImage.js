...
// Asset
import {IMAGE_MEMBER_ARTIST, IMAGE_MEMBER_DANITY, IMAGE_MEMBER_MANAGER, IMAGE_MEMBER_SUPPORTERS} from "../../../assets";
...
class CircleBorderImage extends BaseComponent {
    ...
    renderImage = (image, borderWidth = 0) => {
        ...
        if (Common.isEmpty(source)) {
            return (
                <View style={[this.getCircleStyle(image_size, borderWidth), {backgroundColor: colors.grayDark}]}>
                    <Icon.MemberShipOn size={Layout.UISize(image_size * 0.4)}
                                       color={colors.grayLight}/>
                </View>)
        }
        return (
            <BaseImage style={[this.getCircleStyle(image_size, borderWidth), {backgroundColor: colors.grayLight}]}
                       source={{uri: source}}/>)
    }
    ...
    renderBorderGradient = (total, back, image, borderWidth) => {
        ...
        return (
            <View style={{flex: 1}}>
                ...
                <View style={[this.getCircleStyle(back, borderWidth), {backgroundColor: colors.background}]}/>
    ...
    renderGrade = _ => {
        ...
        return !Common.isEmpty(image) &&
            <View style={[s.layout_grade, {right: gradeRight}]}>
                <BaseImage style={[s.grade_icon, {width: Layout.UISize(gradeSize), height: Layout.UISize(gradeSize)}]}
                           source={image}/>
            </View>
    }
    ...
    render() {
        const {
            size, borderWidth,
            isUseBorder, isUseGradient, isUseGrade
        } = this.props;
        if (isUseBorder) {
            ...
            return (
                <View style={{width: totalSize, height: totalSize}}>
                    {isUseGradient
                        ? this.renderBorderGradient(totalSize, Layout.UISize(back), Layout.UISize(size), Layout.UISize(borderWidth))
                        : this.renderBorderNormal(totalSize, Layout.UISize(back), Layout.UISize(size), Layout.UISize(borderWidth))}
