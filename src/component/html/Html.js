...
class Html extends BaseComponent {
    ...
    shouldComponentUpdate(nextProps, nextState, nextContext) {
        if (Common.shouldUpdate(this.props, nextProps, [ 'widthPadding', 'enableImageView', 'translateLanguage'])) return true;
        if (Common.shouldUpdate(this.props, nextProps, ['content'])) {
            //this.setContent(nextProps.content)
            this.translateChange(nextProps)
            return true;
        };
        if (Common.shouldUpdate(this.state, nextState, ['trans_content',])) return true;
