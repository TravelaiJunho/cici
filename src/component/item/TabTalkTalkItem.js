...
// API
import {declareItem, getCommentCount, like} from '../../data/http/TabTalkTalk';
import TranslateButton from "../button/TranslateButton";
import BaseModalImage from "../_base/BaseModalImage";
...
class TabTalkTalkItem extends BaseComponent {
    ...
    shouldComponentUpdate(nextProps, nextState, nextContext) {
        ...
        if (Common.shouldUpdate(this.props, nextProps, ['item'])){
            const {iLike, likes, count_comments} = nextProps.item;
            this.setState({
                // Like
                isLike: iLike,
                likeCount: likes,
        ...
        if (Common.shouldUpdate(this.props, nextProps, ['myItem','isDetail','profile', 'reasonList', 'translateLanguage'])) return true;
        if (Common.shouldUpdate(this.state, nextState, ['item','isShowConfirm', 'confirmMessage', 'mediaIndex', 'mediaMeta', 'isLike', 'likeCount', 'commentCount', 'autoTranslate', 'disableAutoTrans'])) return true;
        return false;
    }
    ...
    onPost = _ => {
        ...
        const {item} = this.state;
        navRef.current.navigate(Screen.SCREEN_ACTIVITY.POST, {
            type: Screen.STACK_COMMUNITY.TAB_TALK_TALK,
            item: item,
            reload: refresh
        });
    }
    ...
    renderNormalMainText = (main, tag) =>
        <View style={s.layout_contents}>
            ...
            <AttributeText text={main} autoTranslate={this.state.autoTranslate}/>
            {/* HashTag */}
    ...
    // Bottom
    renderBottom = _ => {
        const {isLike, likeCount, commentCount, autoTranslate} = this.state;
        const {myItem} = this.props;
        return (
            <View style={s.layout_bottom}>
                <View style={s.layout_bottom_inside}>
                    {/* Like */}
                    <TouchableOpacity style={{flexDirection: "row"}}
                                      onPress={this.sendLike}>
                        {this.getLikeIcon(isLike)}
                        <BaseText
                            style={[FontStyle.SubCntWhiteLT, s.layout_bottom_like]}>{Common.numberFormatWithSymbol(likeCount, 1, true)}</BaseText>
                    </TouchableOpacity>
                    {/* Comment */}
                    <TouchableOpacity style={{flexDirection: "row"}}
                        ...
                        <BaseText
                            style={[FontStyle.SubCntWhiteLT, s.layout_bottom_comment]}>{Common.numberFormatWithSymbol(commentCount, 1, true)}</BaseText>
                    </TouchableOpacity>
                </View>

                {/* Translate */}
                {
                    (Common.isEmpty(myItem)) &&
                    <TranslateButton
                        enabled={autoTranslate}
                        onEnabled={this.onEnableTranslate}
                    />
                }
    ...
    renderDisable = () => {
        const {myItem} = this.props;
        const {disableAutoTrans} = this.state;
        if(Common.isEmpty(myItem) || !myItem.is_disabled) {
            return null;
        }

        return (
            <View style={s.layout_disable}>
                <View style={s.layout_disable_title}>
                    <BaseText style={FontStyle.CntTitleWhiteLH}>{localize.success.post.blind}</BaseText>
                    {/* Translate */}
                    <TranslateButton
                        enabled={disableAutoTrans}
                        onEnabled={enabled=>{
                            this.setState({
                                disableAutoTrans: enabled
                            })
                        }}
                    />
                </View>
                <View style={s.layout_disable_comment}>
                    <BaseTransText autoTranslate={disableAutoTrans} style={FontStyle.Cnt13WhiteLN}>{myItem.disable_comment}</BaseTransText>
                </View>
            </View>
        )
    }
    ...
    render() {
        ...
        const {item} = this.state;
        if (Common.isEmpty(item)) return null;
        const {isShowConfirm, confirmMessage, autoTranslate} = this.state;
...
export default connect(mapStateToProps, mapDispatchToProps)(React.memo(TabTalkTalkItem));
