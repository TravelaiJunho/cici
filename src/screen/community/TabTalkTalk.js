...
import s from "../_style/TabTalkTalk.style";
...
import {getList, setFocusForTalkTalk} from '../../data/redux/action/TabTalkTalk';

////////////////////////////////////////
// CLASS
////////////////////////////////////////

class TabTalkTalk extends BaseScreen {
    ...
    onFocus = _ => {
        ...
        onTabFocus && onTabFocus(Screen.STACK_COMMUNITY.TAB_TALK_TALK);
        // Cover To On
    ...
    ////////////////////
    // RENDER
    renderItem = ({item, index}) => <TalkTalkItem key={index}
                                                  item={item}
...
const mapStateToProps = (state) => {
    return {
        ...
        // Focus
        isFocus: state.tab_talk_talk.get('isFocus'),
        // Filter
        hashTag: state.tab_talk_talk.get('hashTag'),
        publisher: state.tab_talk_talk.get('publisher'),
        // Tab TalkTalk
        status: state.tab_talk_talk.get('status'),
        list: state.tab_talk_talk.get('list'),
        page: state.tab_talk_talk.get('page'),
        isRefresh: state.tab_talk_talk.get('isRefresh'),
    };
};
...
const mapDispatchToProps = (dispatch) => {
    return {
        sendFocus: _ => {
            return dispatch(setFocusForTalkTalk());
        },
...
export default connect(mapStateToProps, mapDispatchToProps)(TabTalkTalk);
