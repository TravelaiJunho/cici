...
import s from "../../_style/InquireList.style";
...
import {getList} from '../../../data/redux/action/Inquire';

////////////////////////////////////////
// CLASS
////////////////////////////////////////

class InquireList extends BaseScreen {
    ...
    // Event
    onSelectItem = item => {
        this.props.navigation.navigate(Screen.SCREEN_ACTIVITY.INQUIRE_DETAIL, {id: item.id});
    }
    ...
    renderCategory = type => {
        switch (type) {
            case INQUIRE.ANSWER_WAIT:
                return (
                    <View style={{flexDirection: "row"}}>
                        <View style={[s.layout_category, {borderColor: colors.gray}]}>
                            <BaseText style={FontStyle.Cnt13WhiteCN}>{localize.more.inquire.answer_wait}</BaseText>
                        </View>
                    </View>);

            case INQUIRE.ANSWER_COMPLETE:
                return (
                    <View style={{flexDirection: "row"}}>
                        <View style={[s.layout_category, {borderColor: colors.mint}]}>
    ...
    renderItem = ({item, index}) => {
        const {status, title, question, created_at} = item;
        return (
            <TouchableOpacity style={s.layout_item}
                ...
                {/* Contents */}
                <BaseText style={[FontStyle.Cnt13WhiteLN, s.layout_contents]} numberOfLines={1}>{question}</BaseText>
                {/* Date */}
...
const mapStateToProps = (state) => {
    return {
        ...
        // Inquire
        status: state.inquire.get('status'),
        list: state.inquire.get('list'),
        page: state.inquire.get('page'),
        isRefresh: state.inquire.get('isRefresh'),
    };
};
...
export default connect(mapStateToProps, mapDispatchToProps)(InquireList);