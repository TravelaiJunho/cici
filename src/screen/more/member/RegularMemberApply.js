...
const CheckBox = (props) => {
  ...
  return (
    ...
    >
      <BaseText style={FontStyle.Cnt13WhiteLN}>
        {item.title + (select ? localize.more.membershiprequest.selected : "")}
      </BaseText>
    </TouchableOpacity>
  );
};
...
class QuestionBox extends Component {
  constructor(props) {
    super(props);
    let preValue = Common.isEmpty(props.item.answer)
      ? ""
      : props.item.answer + "";
    this.state = {
      valueData: preValue,
  ...
  checkValue = () => {
    ...
    let value = Common.isEmpty(this.state.valueData)
      ? null
  ...
  resetValueData = () => {
    this.setErrorMsg(false);
    let value = Common.isEmpty(this.props.item.answer)
      ? null
    ...
    return (
      <View style={s.question_text}>
        <FlatInput
          ...
          inputStyle={{
            color: this.props.editable ? colors.white : colors.grayDark,
          }}
  ...
  renderTextDate = (item) => {
    ...
    return (
      <TouchableOpacity
        ...
        style={[
          s.question_date,
          { borderBottomColor: errorMsg ? colors.orange : colors.white },
        ]}
        onPress={() => {
      ...
      >
        ...
        <BaseText
          style={[
            FontStyle.Cnt13GrayDkCN,
            {
              color: !Common.isEmpty(valueData)
                ? this.props.editable
                  ? colors.white
                  : colors.grayDark
                : colors.gray,
        ...
        >
          {!Common.isEmpty(valueData)
            ? valueData
            : localize.more.membershiprequest.input_answer}
        </BaseText>
        {errorMsg && (
          <BaseText
            style={[
              FontStyle.CntNoticeOrangeLT,
              { position: "absolute", bottom: Layout.UISize(-25) },
            ]}
          >
      ...
      <TouchableOpacity style={s.question_image}>
        {Common.isEmpty(valueData) ? (
          <View style={s.question_image_container}>
            <BaseImage
              style={s.question_image_box}
              source={{
                uri: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSCW4tbiTnEpXwd1oRms9yXuyZSe-bI-u3R2w&usqp=CAU",
              }}
            />
            <View style={s.question_image_cancel}>
              <BaseImage style={s.image_close} source={IMAGE_CANCEL} />
            </View>
          </View>
        ) : (
...
class RegularMemberApply extends BaseScreen {
  ...
  constructor(props) {
    ...
    this.editable = true;
    if (
      !Common.isEmpty(this.props.profile.levelup_request_status) &&
      !Common.isEmpty(
        this.props.profile.levelup_request_status.latest_request_data
      )
    ) {
  ...
  onApply = (_) => {
    ...
    this.state.formData.map((item, index) => {
      ...
      if (!Common.isEmpty(res.data)) {
        ...
        if (res.require) checkpass = false;
      }
    });
    ...
    userLevelUpRequest(formData, (success, code, message, data) => {
      ...
      if (success) {
        this.setShowConfirm(
          true,
          localize.success.membership.request_success,
          () => {
  ...
  renderItem = (item, index) => {
    ...
    switch (item.answerType) {
      ...
      case ANSWER_TYPE.I: {
        ...
        if (!Common.isEmpty(item.answer) && !this.state.editable)
          imageItem.push({ id: 0, image_url: item.answer });
        /*
                item.answer.map(im=>{
                    item.push(im)
                })
      ...
      return (
        <View style={s.top_request}>
          <BaseText style={FontStyle.Cnt13WhiteLT}>
            {localize.more.membershiprequest.membership_desc}
          </BaseText>
        </View>
      );
    }
        ...
        return (
          <View style={s.top_reject_container}>
            <View style={s.top_reject_inner}>
              <BaseText
                style={[
                  FontStyle.CatOrangeCH,
                  { marginBottom: Layout.UISize(15) },
                ]}
              >
                {localize.more.membershiprequest.reject}
              </BaseText>
              <BaseText style={FontStyle.Cnt13WhiteLT}>
                {localize.more.membershiprequest.reject_desc}
              </BaseText>
              <BaseText style={FontStyle.Cnt13WhiteLT}>
                {localize.more.membershiprequest.reject_time}
                {date}
              </BaseText>
              <BaseText style={FontStyle.Cnt13WhiteLT}>
                ...
                <BaseTransText
                  ...
                  style={FontStyle.Btn13OrangeLN}
                >
                  {this.state.transRejectComment
                    ? this.state.transRejectComment
        ...
        return (
          <View style={s.top_applying_container}>
            <View style={s.top_applying_inner}>
              <BaseText
                style={[
                  FontStyle.Cnt13MintCB,
                  { marginBottom: Layout.UISize(15) },
                ]}
              >
                {localize.more.membershiprequest.applying}
              </BaseText>
              <BaseText style={FontStyle.Cnt13WhiteLT}>
                {localize.more.membershiprequest.applying_desc}
              </BaseText>
            </View>
          </View>
        );
