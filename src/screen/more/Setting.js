...
import {
  ...
  changeLanguage,
} from "../../data/redux/action/Common";
import {
  getAlarmAllow,
  setAlarm,
  setAlarmCancel,
} from "../../data/http/Setting";
...
class Setting extends BaseScreen {
  ...
  shouldComponentUpdate(nextProps, nextState, nextContext) {
    if (
      Common.shouldUpdate(this.state, nextState, [
        "isLoading",
        "isShowConfirm",
        "showSelectLanguage",
        "notificationOnOff",
  ...
  getMenuList = (_) => {
    return [
      ...
      },
      {
        ...
        desc:
          this.props.downloadOption == DOWNLOAD_OPTION.WIFI_ONLY
            ? localize.more.preferences.download_setting_desc
            : localize.more.preferences.download_setting_desc2,
      },
      {
        id: 3,
  ...
  onChangeNotification = () => {
    this.state.notificationOnOff
      ? this.sendSetAlarmCancel()
      : this.sendSetAlarm();
  };
  ...
  onMenu = (item) => {
    switch (item.id) {
      ...
      case 3: {
        this.bottomLanguage &&
          this.bottomLanguage.open(this.props.translateLanguage);
        break;
      }
      case 4: {
  ...
  renderLanguage = (_) => {
    ...
    return (
      ...
      >
        ...
        <SwipeablePanel
          isActive={this.state.showSelectLanguage}
          onlySmall={true}
        ...
        >
          ...
          <View style={s.popEditBtnContainer}>
            <View style={s.popEditBtnTitle}>
              <BaseText style={[FontStyle.Cnt13GrayCB]}>
                {localize.more.preferences.select_language}
              </BaseText>
            </View>
            <View style={s.line} />
            <BaseButton
              titleStyle={FontStyle.CntWhiteCN}
          ...
          <View style={s.popEditBtn}>
            <BaseButton
              titleStyle={FontStyle.BtnGrayCB}
              buttonStyle={s.popEditBtn}
      ...
      return (
        <View>
          <ListItem
            containerStyle={[
              s.listItemContainer,
              { alignItems: "flex-start", borderTopWidth: index == 0 ? 0 : 1 },
            ]}
            onPress={() => {
              this.onMenu(item);
            }}
          >
            <ListItem.Content>
              <ListItem.Title style={FontStyle.CntWhiteLN}>
                {item.name}
              </ListItem.Title>
              <BaseText
                numberOfLines={2}
