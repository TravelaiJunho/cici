...
import s from '../../_style/PostSuggest.style';
...
// API
import {post} from '../../../data/http/Suggest';

////////////////////////////////////////
// CLASS
////////////////////////////////////////

class PostSuggest extends BaseScreen {
    ...
    onResultPost = (success, code, message, data) => {
        ...
        if (success) {
            this.setShowConfirm(true, localize.success.post.suggest_complete, this.onCancel);
        } else {
    ...
    render() {
        ...
        return (
            <KeyboardAvoidingView style={s.container}
                ...
                {/* Header */}
                <BackHeader title={localize.more.suggest.title}
                ...
                <ScrollView bounces={false}
                    ...
                    <View style={s.container_contents}>
                        {/* Information */}
                        <BaseText style={[s.layout_info, FontStyle.Cnt13WhiteLT]}>{localize.more.suggest.text.information}</BaseText>
                        {/* Title */}
                        <View style={s.layout_title}>
                            <FlatInput ref={ref => this.title = ref}
                                       required={true}
                                       maxLength={50}
                                       label={localize.more.suggest.text.title}
                                       placeHolder={localize.more.suggest.hint.title}/>
                        </View>
                        {/* Main Text */}
                        <View style={s.layout_main}>
                            <BaseHelperLayout label={localize.more.suggest.text.contents}
                                              required={true}>
                                {/* Input */}
                                <View style={s.layout_input_main}>
                                    <BaseTextInput style={[FontStyle.Cnt13WhiteLN, s.input_main]}
                                                   multiline={true}
                                                   placeholder={localize.more.suggest.hint.contents}
