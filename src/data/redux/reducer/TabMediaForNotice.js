...
import {NOTICE_TAB_MEDIA_LIST, STATUS} from '../_base/ActionType';
...
export default function TabMediaForNotice(state = initialState, action) {
    switch (action.type) {
        ////////////////////
        // List
        case NOTICE_TAB_MEDIA_LIST.RESET:
            return state.merge({list: []});

        case NOTICE_TAB_MEDIA_LIST.START:
            return state.update('status', status => STATUS.WAITING);

        case NOTICE_TAB_MEDIA_LIST.COMPLETE:
            return state.update('status', status => STATUS.COMPLETE);

        case NOTICE_TAB_MEDIA_LIST.SUCCESS:
            return state.merge({
        ...
        case NOTICE_TAB_MEDIA_LIST.FAILED: {
